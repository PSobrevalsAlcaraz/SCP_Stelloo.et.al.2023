...

{
  ...
  # BiocManager::install("anchors",version = "3.16")
  # require(devtools)
  # install_version("scp", version = "1.0.0")
  ...
  load_pack(c("randomcoloR","expss","svDialogs","dplyr","scater",'ggbreak','viridis',"seqinr","ggpubr","ggplot2","RColorBrewer","scp","magrittr","tidyr","limma","dplyr","proDA","tcltk","anchors","ggplot2","ggrepel","plotly","DEP","reshape2", "ggthemes"))
  ...
  InitialValues <- function(ins.n.obs, ins.Top.scr.thr, ins.Low.scr.thr, ins.booster, 
    ...
    clean <- function(){
      ...
      tclvalue(batch) <- 'Both'
      tclvalue(description) <- 'MOUSE'
      tclvalue(fastafile) <- 'F:\\fasta\\Mouse_20200121.fasta'
...
init <- InitialValues(
  ...
  ins.fastafile = "F:\\fasta\\Mouse_20200121.fasta", ins.description = "MOUSE")
...
new.PSM$Abundance.Median126 <- NA
...
for(i in unique(new.PSM$File.ID)){
  ...
  temp <- colSums(as.matrix(temp[,grep("Abun",colnames(temp))[-1]]),na.rm=T)
  new.PSM$Abundance.Median126[new.PSM$File.ID==i] <- sum(temp, na.rm=T)
...
unique.InputFiles <- cbind(unique.InputFiles$File.ID,"Abundance.Median126",unique.InputFiles$Experiment)
...
new.InputFiles <- new.InputFiles[order(new.InputFiles$Experiment),]
...
{
  ## -----------------------------------------------------------------------------
  if(class(try(readSCP(featureData = new.PSM,
                       colData = new.InputFiles,
                       channelCol = "Channels",
                       batchCol = "File.ID",
                       removeEmptyCols = TRUE, verbose = F), silent = T)) == "try-error"){
    featureData = new.PSM
    colData = new.InputFiles
    channelCol = "Channels"
    batchCol = "File.ID"
    ...
    if (is.null(suffix)) {
      suffix <- ecol}
    if (verbose) {
      message("Loading data as a 'SingleCellExperiment' object")}
    scp <- readSingleCellExperiment(table = featureData, ecol = ecol)
    ...
    if (verbose) {
      message(paste0("Splitting data based on '", batchCol, 
                     "'"))}
    ...
    if (verbose) {
      message(paste0("Formatting sample metadata (colData)"))}
    cd <- DataFrame(row.names = unlist(lapply(scp, colnames)))
    rownames(colData) <- paste0(colData[, batchCol], colData[, channelCol])
    cd <- cbind(cd, colData[rownames(cd), ])
    if (verbose) {
      message("Formatting data as a 'QFeatures' object")}
    scp <- QFeatures(experiments = scp, colData = cd)
